...
def get_latency_with_input(spikes, input_electrode_number, output_electrode_number):
    # Filter spikes for input and output electrodes
    input_spikes = spikes[spikes[:, 0] == input_electrode_number]
    output_spikes = spikes[spikes[:, 0] == output_electrode_number]
    ...
    for spike in sorted_spikes:
        ...
        if spike[0] == input_electrode_number:
            ...
            if input_time is not None and (current_time - input_time) < 10:
                # Also record it as an output of the previous input spike
...
def get_latency_with_extremum(spikes_extremum, unit_ids, unit_pre, unit_post):
    ...
    print("Input",len(input_spikes))
    print("Output",len(output_spikes))
    # Sort spikes by time
    #input_spikes = input_spikes[np.argsort(input_spikes[:, 1])]
    #output_spikes = output_spikes[np.argsort(output_spikes[:, 1])]
    ...
    # Preallocate latency array with an upper bound on size
    max_possible_size = len(input_spikes) + len(output_spikes)
    latency = np.zeros(max_possible_size, dtype=[('input spike', 'i4'), ('spike time', 'f8'), 
                                                 ('latency', 'f4'), ('category', 'U6')])
    ...
    # Iterate through both input and output spikes by merge sort technique
    while input_index < len(input_spikes) and output_index < len(output_spikes):
        input_spike_time = input_spikes.iloc[input_index]['Spike_Time']
        output_spike_time = output_spikes.iloc[output_index]['Spike_Time']
    ...
    # Process remaining input spikes
    while input_index < len(input_spikes):
        input_time = input_spikes.iloc[input_index]['Spike_Time']
        latency[index] = (input_spike_count, input_time, 0, "input")
        input_spike_count += 1
        input_index += 1
        index += 1
    ...
    # Process remaining output spikes
    while output_index < len(output_spikes):
        output_time = output_spikes.iloc[output_index]['Spike_Time']
        if input_index > 0:  # There has been at least one input spike
            latency[index] = (input_spike_count, output_time, output_time - input_time, "output")
        output_index += 1
        index += 1
...
def plot_latency_and_location(save_path, spikes, input_ids, output_ids, elec_to_color, unit_pre, unit_post, area):
    include_input_in_latency = True
    ...
    for idx, input_id in enumerate(input_ids):
        ...
        for output_electrode in output_electrodes_filtered:
            ...
            if output_color is not None:
                ...
                input_before = latency[latency['category'] == 'input']
                output_before = latency[latency['category'] == 'output']
                with open(os.path.join(save_path, f"area_{area}_input_el_{input_id}_output_el_{int(output_electrode)}.pkl"), 'wb') as f:
                    pickle.dump(latency, f)

                ax1.scatter(input_before['latency'], input_before['spike time'] / 1000, s=7, label='output', color=input_color)
                ax1.scatter(output_before['latency'], output_before['spike time'] / 1000, s=7, label='output', color=output_color)

            plt.savefig(os.path.join(save_path, f"area_{area}_input_el_{input_id}_unit_pre_{unit_pre}_output_el_{int(output_electrode)}_unit_post_{unit_post}_STTRP.pdf"), format='pdf')
            plt.savefig(os.path.join(save_path, f"area_{area}_input_el_{input_id}_unit_pre_{unit_pre}_output_el_{int(output_electrode)}_unit_post_{unit_post}_STTRP.png"), format='png')
            plt.close()
        #plt.show()
        ...
        ax2.set_ylim(0, 120)
        ax2.legend()
        plt.savefig(os.path.join(save_path, f"area_{area}_input_el_{input_id}_color_code.pdf"), format='pdf', dpi=300)
        plt.savefig(os.path.join(save_path, f"area_{area}_input_el_{input_id}_color_code.png"), format='png')
        plt.close()

def plot_latency_and_location_with_extremum(save_path, filename, input_ids, output_ids, elec_to_color, unit_pre, unit_post, unit_ids, spikes_extremum, lag):
    #include_input_in_latency = True
    #gain = 6.3
    for idx, input_id in enumerate(input_ids):
        #if include_input_in_latency:
        #    output_ids = list(set(output_ids + [id for id in input_ids if id != input_id]))
        num_spikes_threshold = 10
        input_electrode = np.array([input_id], dtype=float)
        input_color = elec_to_color['color'][elec_to_color['electrode'].index(input_id)]
        #input_spikes = spikes[spikes[:, 0] == input_electrode]
        ...
        for output_electrode in output_electrodes_filtered:
            output_color = elec_to_color['color'][elec_to_color['electrode'].index(output_electrode)]
            if output_color is not None:
                latency = get_latency_with_extremum_V2(spikes_extremum, unit_ids, unit_pre, unit_post)
                latency = latency[latency['latency'] < 10]
                
                input_before = latency[latency['category'] == 'input']
                output_before = latency[latency['category'] == 'output']

                if len(output_before) < num_spikes_threshold:
                    return None
                else:
                    ...
                    ax1.scatter(latency_extremum['latency'], latency_extremum['spike time'] / 1000, s=20, label='output extremum', color='pink')
        ...
        ax2.legend()
        #plt.close(fig2)
        plt.savefig(os.path.join(save_path, f"{filename[:-3]}_input_el_{input_id}_unit_pre_{unit_pre}_output_el_{int(output_electrode)}_unit_post_{unit_post}_color_code.pdf"), format='pdf', dpi=300)
        plt.savefig(os.path.join(save_path, f"{filename[:-3]}_input_el_{input_id}_unit_pre_{unit_pre}_output_el_{int(output_electrode)}_unit_post_{unit_post}_color_code.png"), format='png')
        #plt.show()
        plt.close()
    #return
    return latency
...
def plot_latency_and_location_with_extremum_without_small(save_path, filename, input_ids, output_ids, electrodes_pre, electrodes_post, unit_pre, unit_post, unit_ids, spikes_extremum, lag):
    for idx, input_id in enumerate(input_ids):
        num_spikes_threshold = 10
        #input_electrode = np.array([input_id], dtype=float)
        #input_color = elec_to_color['color'][elec_to_color['electrode'].index(input_id)]
        ...
        for output_electrode in output_electrodes_filtered:
            ...
            elec_to_color = plot_color_coded_electrodes(save_path, unit_pre, unit_post, electrodes_pre, electrodes_post, filename)
            for electrode in output_ids:
                x, y = convert_elno_to_xy(electrode)
                ax2.scatter(y, x, color=elec_to_color['color'][elec_to_color['electrode'].index(electrode)])
                ax2.scatter(y, x, color='red', marker='x', s=30, label=f'Output {electrode}')
