...
class SortingExtractor(ABCExtractor):
    ...
    def load_data(
        ...
        # Load recording
        try:
            self.recording = si.read_maxwell(file_path=os.path.join(self.input_path, self.filename),use_names_as_ids=True)
        ...
        if os.path.exists(os.path.join(self.sorting_output_path, "analyzer_curated")):
                ...
                self.get_spike_matrix_extremum()
                self.get_spike_matrix()
                self.get_units_to_electrode_ids()
                if plot_sorting:
    ...
    def perform_spike_sorting(self):
        ...
        # Set parameters
        sorter_params = si.get_default_sorter_params(sorter_name_or_class="spykingcircus2")
        ...
        sorter_params["selection"]["method"] = "smart_sampling_amplitudes"
        sorter_params["selection"]["min_n_peaks"] = 20000
        sorter_params["job_kwargs"]["n_jobs"] = self.n_jobs
        sorter_params["matching"]["method"] = "circus-omp-svd"
        sorter_params["apply_motion_correction"] = False
        sorter_params["selection"]["n_peaks_per_channel"] = 5000
        ...
        # perform spike sorting
        sorting = si.run_sorter(sorter_name='spykingcircus2',
    ...
    def curate_spike_sorting(self, check_isi=False):
        ...
        if os.path.exists(os.path.join(self.sorting_output_path, "analyzer_curated")):
            ...
            console.info("Computing sparsity...")   
            ...
            console.info("Correlograms - done")
            ...
            #self.sorting, self.analyzer, self.good_unit_ids = self.merge_spike_sorting()   
    ...
    def merge_spike_sorting(self, threshold=0.8):    
        ...
        if selected_merges:
            ...
            self.good_unit_ids = self.sorting.get_unit_ids()
            console.info("No clusters were merged.")
    ...
    def get_electrode_channel_mapping(self, raw_data) -> np.array:
        ...
        with open(os.path.join(self.sorting_output_path, "analyzer_curated/recording_info/recording_attributes.json"),
            ...
            #channel_inds = np.arange(0, len(rec_attributes["channel_ids"]), dtype=int)
            channel_ids = self.analyzer.channel_ids
            channel_inds = self.analyzer.channel_ids_to_indices(channel_ids)

            # Create a mapping dictionary
            self.electrode_channel_mapping = dict(zip(channel_inds, self.electrodes))
    ...
    def get_units_to_channel_ids(self):
        ...
        print(unit_id_to_channel_ids.keys())
    ...
    def get_units_to_electrode_ids(self):
        ...
        """
        unit_id_to_channel_ids = self.analyzer.sparsity.unit_id_to_channel_indices
        print("Keys of unit id to channel ids ", unit_id_to_channel_ids.keys())
        self.get_electrode_channel_mapping(raw_data=None)
        self.unit_id_to_electrode_ids = {key: [self.electrode_channel_mapping[x] for x in value] for key, value
    ...
    """   
    def filter_good_units_based_on_extremum(self):
        unique_extremum_channel = {}
        seen_values = set()
        extremum_channel = si.get_template_extremum_channel(self.analyzer, outputs = 'index')
        ...
        self.good_unit_ids = [unit for unit in self.good_unit_ids if unit in unique_extremum_channel_keys]
        return self.good_unit_ids
    """ 
    ...
    def get_spike_matrix_extremum(self):
        ...
        extremum_channel = si.get_template_extremum_channel(self.analyzer, outputs = 'index')
        #print("This is extremum channel: ",extremum_channel)
        ...
        time_in_ms = (spike_df["sample_index"] / self.sample_frequency) * 1000
        
        spike_df["time"] = time_in_ms
        print("electrode to channel mapping", self.electrode_channel_mapping)
        # Add the 'Electrode' column to the DataFrame
        spike_df['Electrode'] = spike_df['channel_index'].map(self.electrode_channel_mapping)

        unit_ids = self.analyzer.unit_ids

            
        spike_df['Unit_id'] = spike_df['unit_index'].map(lambda x: unit_ids[x])
            
        # Convert to a numpy array
        df = spike_df[["Electrode", "time", "Unit_id"]]
        ...
        print("Spike matrix extremum generated successfully.")
        return self.spike_mat_extremum
    ...
    def get_spike_matrix(self):
        ...
        if debug:
            ...
            all_spikes['Electrode'] = all_spikes['channel_index'].map(self.electrode_channel_mapping)
            # Convert sample indices to milliseconds
        ...
        print("Spike matrix generated successfully.")
